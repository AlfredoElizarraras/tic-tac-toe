...
RSpec.describe Main do
  let(:instructions_1) { "Welcome to Tic-tac-toe\nSelect a number to replace with the given mark:" }
  let(:instructions) { instructions_1 + "\nPlayer 1: 'X'\nPlayer 2: 'O'\n\n\n" }
  ...
  describe '#assign_names' do
    ...
    it 'create an instance of a player and if the parameter given is true assign a name to player 1' do
      allow($stdin).to receive(:gets).and_return p1_name
      main.assign_names(true)
      expect(main.player1.name).to eql(p1_name)
    end

    it 'create an instance of a player and if the parameter given is false assign a name to player 2' do
      allow($stdin).to receive(:gets).and_return p2_name
      main.assign_names(false)
      expect(main.player2.name).to eql(p2_name)
    end
  end
  ...
  describe '#game_start' do
    it 'outputs that the player 1 won and a game over message' do
      main = Main.new
      allow($stdin).to receive(:gets).and_return(p1_name, p2_name, '1', '2', '3', '4', '5', '6', '7')
      expect do
        main.game_start
    ...
    it 'outputs that the player 2 won and a game over message' do
      main = Main.new
      allow($stdin).to receive(:gets).and_return(p1_name, p2_name, '1', '2', '3', '5', '4', '8')
      expect do
        main.game_start
    ...
    it 'outputs that there was a tie and a game over message' do
      main = Main.new
      allow($stdin).to receive(:gets).and_return(p1_name, p2_name, '1', '2', '3', '5', '4', '6', '9', '7', '8')
      expect do
        main.game_start
